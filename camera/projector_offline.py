...
def joint_input(joint_state,trans_f = 0):
    ...
    cv2.putText(img=image, text="Mocap Time: "+str(row[0]),org=(10,25) ,
    fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.8, color=(0, 0, 255),thickness=1)
    cv2.putText(img=image, text="  |  Frame No. " + str(frame_num),org=(275,25) ,
    fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.8, color=(0, 0, 255),thickness=1)
...
#        next(reader)
    ...
    ret, image = cap.read()
    buffer_pose = [joint_data.copy(), joint_data.copy(), joint_data.copy(), joint_data.copy(), joint_data.copy()]
    for i, row in enumerate(reader):
        frame_num = int(cap.get(cv2.CAP_PROP_POS_FRAMES))
        #frame_rate = int(cap.get(cv2.CAP_PROP_FPS))
        time_stamp = round(slope*frame_num + offset)

        for local_pos, csv_pos in csv_joints:
            joint_data[local_pos].append(float(row[csv_pos])/100)
            joint_data[local_pos].append(float(row[csv_pos+1])/100)
            joint_data[local_pos].append(float(row[csv_pos+2])/100)
        for i in range(4):
            buffer_pose[i] = buffer_pose[i+1].copy()

        buffer_pose[4] = joint_data.copy()
        joint_data = [[] for i in range(32)]
        ...
        if  time_stamp > 4 + round((float(row[0])*100)):
            continue
        ...
        ret, image = cap.read()
        b_img = image.copy()
        print("Mocap Time: ",row[0], "  |  Frame No. ",frame_num)
        # Extract the position data (assuming it's in the first 3 columns)
        joint_data = buffer_pose[2]
        i = 0

        """
        for joint in joints_info:
            print(joint, " : ", joint_data[i])
            i +=1
        """
        ...
        if interpolate_flag == 1:
            for i in [0,1,3,4]:
                rot1 = body_rotation(buffer_pose[i],13*np.pi/20,0,1,0)
                rot2 = body_rotation(rot1,90*np.pi/500 * (-1),1,0,0)
                joint_input(rot2,1)
